...
export default class Victory extends Phaser.State {
  ...
  create() {
    ...
    const style = { font: `${Settings.VictoryFontSize()}px ${Settings.FontStyle()}`, fill: Settings.VictoryFontColor() };
    const style2 = { font: `${Settings.FontSize()}px ${Settings.FontStyle()}`, fill: Settings.VictoryFontColor() };
    const style3 = { font: `${Settings.FontSize()}px ${Settings.FontStyle()}`, fill: Settings.FontColor() };
    this.text = this.game.add.text(0,0, 'Victory', style);
    this.text.x = (this.game.width - this.text.width) / 2;
    this.text.y = (this.game.height - this.text.height) / 2;
    this.text.fixedToCamera = true;

    this.text2 = this.game.add.text(0,0, 'And you were heard from again', style2)
    ...
    this.text4 = this.game.add.text(0,0, 'press v to restart', style3);
    ...
    this.xKey = this.game.input.keyboard.addKey(Phaser.Keyboard.V);
  }
